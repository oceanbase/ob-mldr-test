...
class QueryArgs:
    ...
    all: bool = field(default=False, metadata={'help': 'Process all supported languages.'})
    debug_query_id: str = field(default='', metadata={'help': 'Debug specific query ID (e.g., q-en-126) to show execution details.'})


def get_default_query_result_dir(languages):
    """根据语言列表生成默认的查询结果存储目录"""
    ...
    # 如果只有一个语言，使用该语言作为目录名
    if len(languages) == 1:
        return f"./{languages[0]}-results"
    
    # 如果有多个语言，使用语言列表作为目录名
    lang_str = "-".join(languages)
    return f"./{lang_str}-results"
...
def check_query_types(query_types):
    available_query_types = ['bm25', 'dense', 'sparse', 'colbert', 'dense_bm25']
...
def load_corpus(lang: str, streaming: bool = False):
    ...
    try:
        ...
        # 如果本地模块不存在，回退到原始方法
        logging.debug("本地MLDR模块不存在，使用远程加载...")
        if streaming:
...
def get_queries_and_qids(lang: str, streaming: bool = False):
    # 使用本地MLDR.py脚本加载数据
    try:
        from .load_mldr_local import get_queries_and_qids_local
        return get_queries_and_qids_local(lang, streaming)
    except ImportError:
        # 如果本地模块不存在，回退到原始方法
        logging.debug("本地MLDR模块不存在，使用远程加载...")
        if streaming:
...
def save_result_with_scores(search_results, result_save_path: str, qids: list, max_hits: int):
    """保存包含分数的查询结果到文件
    
    Args:
        search_results: List[Tuple[str, List[JScoredDoc]]] - 查询结果列表
        result_save_path: str - 保存路径
        qids: list - 查询ID列表
        max_hits: int - 最大结果数量
    """
    # 创建分数保存路径（使用独立的score.txt文件）
    base_path = result_save_path.rsplit('.', 1)[0]  # 去掉扩展名
    scores_save_path = f"{base_path}_score.txt"
    
    logging.debug(f"保存查询分数到: {scores_save_path}")
    
    with open(scores_save_path, 'w', encoding='utf-8') as f:
        for topic, hits in search_results:
            for rank, hit in enumerate(hits, 1):
                if rank > max_hits:
                    break
                # 格式: query_id doc_id rank score
                f.write(f"{topic} {hit.docid} {rank} {hit.score:.6f}\n")
    
    # 同时保存标准的TREC格式结果
    save_result(search_results, result_save_path, qids, max_hits)
...
def get_all_part_begin_ends(total_row_count: int):
    ...
    while pos_now < total_row_count:
        ...
        if pos_now >= new_pos or new_pos > total_row_count:
            logging.warning("Invalid value. Input again.")
            continue
...
def apply_sparse(table, query_embedding: dict, max_hits: int):
        try:
            from infinitypy import SparseVector  # type: ignore
            return table.match_sparse("sparse_col", SparseVector(**query_embedding), "ip", max_hits, {"alpha": "0.9", "beta": "0.5"})
        except ImportError as e:
            raise ImportError(f"Sparse vector dependencies not installed: {e}")
...
class FakeBGEM3FlagModelColBERTV2:
    def __init__(self):
        try:
            ...
            raise ImportError(f"ColBERTv2 dependencies not installed: {e}")
...
class FakeBGEM3FlagModelJinaColBERT:
    def __init__(self):
        try:
            ...
            raise ImportError(f"ColBERT dependencies not installed: {e}")
...
class FakeBGEM3FlagModel:
    def __init__(self, use_fp16: bool):
        try:
            ...
            raise ImportError(f"FlagEmbedding not installed: {e}")
